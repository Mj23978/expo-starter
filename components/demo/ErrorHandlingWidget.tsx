...
const errorScenarios = {
  ...
  validation: () =>
    Promise.reject(new Error("Validation failed: Invalid data provided")),
  timeout: () =>
    ...
    <Card className="w-full">
      <CardHeader>
        ...
        <Text className="text-sm text-muted-foreground">
          Comprehensive error handling with React Query, global state, and toast
          notifications
        </Text>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <View className="bg-muted/50 p-3 rounded-lg">
          ...
          <View className="space-y-1">
            ...
            <Text className="text-xs">
              • Custom retry logic and failure counting
            </Text>
          </View>
        </View>
        ...
        {(isError || errorMutation.error || globalError) && (
          <View className="bg-muted/30 p-3 rounded-lg">
            ...
            <View className="space-y-1">
              <Text className="text-xs">
                Query Error: {isError ? "Yes" : "No"}
              </Text>
              <Text className="text-xs">
                Mutation Error: {errorMutation.error ? "Yes" : "No"}
              </Text>
              <Text className="text-xs">
                Global Error: {globalError ? "Yes" : "No"}
              </Text>
              <Text className="text-xs">Failure Count: {failureCount}</Text>
              <Text className="text-xs">
                Selected Scenario: {selectedError}
              </Text>
            </View>
          </View>
        )}
